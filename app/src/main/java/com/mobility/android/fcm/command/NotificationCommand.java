...
import com.shopit.android.BuildConfig;
import com.shopit.android.R;
import com.shopit.android.receiver.NotificationReceiver;
import com.shopit.android.ui.MainActivity;
import com.shopit.android.util.Utils;
...
public class NotificationCommand implements FcmCommand {
    ...
    private void processCommand(Context context, NotificationCommandModel command) {
        ...
        if (TextUtils.isEmpty(dialogTitle) || TextUtils.isEmpty(dialogText)) {
            ...
            // Use a dialog
            intent = new Intent(context, MainActivity.class).setFlags(
                    Intent.FLAG_ACTIVITY_CLEAR_TOP |
                            Intent.FLAG_ACTIVITY_SINGLE_TOP |
                            Intent.FLAG_ACTIVITY_CLEAR_TASK);

            intent.putExtra(MainActivity.EXTRA_DIALOG_TITLE, dialogTitle);
            intent.putExtra(MainActivity.EXTRA_DIALOG_MESSAGE, dialogText);

            intent.putExtra(MainActivity.EXTRA_DIALOG_YES,
                    TextUtils.isEmpty(dialogYes) ? "OK" : dialogYes);
            intent.putExtra(MainActivity.EXTRA_DIALOG_NO,
                    TextUtils.isEmpty(dialogNo) ? "" : dialogNo);
            intent.putExtra(MainActivity.EXTRA_DIALOG_URL,
        ...
        // TODO: 10/10/16 Icon
        int iconResource = 0; /*R.drawable.ic_info_outline_white_24dp;
        if (!TextUtils.isEmpty(command.iconType)) {
            switch (command.iconType) {
                case "info":
                    ...
                    break;
                case "warning":
                    iconResource = R.drawable.ic_warning_white_48dp;
                    break;
            }
        }*/
        ...
        cancelIntent.setAction(NotificationReceiver.ACTION_HIDE_FCM_NOTIFICATION);
        ...
        alarmManager.set(AlarmManager.RTC_WAKEUP, command.expiry * 1000L, pendingIntent);
    }
}
